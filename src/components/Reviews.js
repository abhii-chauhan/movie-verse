    ...
    <div className='mt-4 border-t-2 border-gray-700 w-full'>
        ...
        <button onClick={sendReview} className='bg-green-600 flex justify-center  w-full p-2'>
            ...
            <div className='mt-4'>
                {data.map((e, i) => {
                    return(
                        <div className='bg-red-900 bg-opacity-25 p-2 w-full mt-2 border-b border-white-500' key ={i}>
                            <div className='flex items-centre'>
                            <p className='text-blue-500'>{e.name}</p>
                            <p className='ml-3 text-sm'>({new Date(e.timestamp).toLocaleTimeString()})</p>
                            </div>
                            <ReactStars
                            size={15}
                            half={true}
